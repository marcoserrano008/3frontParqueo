...
import '../../styles/ParqueoFormulario/EditorEspacios.css'
// import Modal from './Modal';
import axiosClient from '../../axios-client';

const EditorEspacios = ({ onParqueoClick }) => {
  ...
  return (
    <div className="parqueo">
      <div className="grillaTop">
        <div className="cell fila-titulo"></div>
        {[...Array(12)].map((_, j) => {
          return (
        ...
        {[...Array(2)].map((fila, i) => {
          return [
            ...
            [...Array(12)].map((columna, j) => {
              ...
              const celdaClassName = `cell state-${estadoEspacio}`;
              return (
                <div
                  key={`${i}-${j}`}
        ...
        <div className="grillaBot" key={`fila-${fila}`}>
          {[...Array(2)].map((_, i) => (
            <React.Fragment key={`${fila}-${i}`}>
              <div className="cell fila-titulo">{fila + i}</div>
              {[...Array(12)].map((_, j) => {
